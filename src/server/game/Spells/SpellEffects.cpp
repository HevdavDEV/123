    ...
    for (uint32 count = 0; count < numGuardians; ++count)
    {
        ...
        if (summon->GetEntry() == 27893)
        {
            if (uint32 weapon = m_caster->GetUInt32Value(PLAYER_VISIBLE_ITEM_16_ENTRYID))
            {
                ...
                summon->SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, caster->GetWeaponDamageRange(BASE_ATTACK, MINDAMAGE));
                summon->SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, caster->GetWeaponDamageRange(BASE_ATTACK, MINDAMAGE));
            }
            else

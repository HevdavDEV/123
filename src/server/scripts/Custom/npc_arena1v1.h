...
static bool Arena1v1CheckTalents(Player* player)
{
	...
	if(count >= 36)
	{
		ChatHandler(player->GetSession()).SendSysMessage("You can not join because you have used more than 35 talent points in a prohibited branch (healer). Please edit your talents.");
		return false;
	}
	else
		return true;

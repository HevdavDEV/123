...
bool Transmogrification::SuitableForTransmogrification(Player* player, ItemTemplate const* proto)
{
    ...
    if (!CanTransmogUnusableItems() && player->CanUseItem(proto) != EQUIP_ERR_OK)
        return false;
    return true;
}
/*
...
bool Transmogrification::CanTransmogUnusableItems() const
{
    return AllowNonusableItems;
}
bool Transmogrification::GetAllowMixedWeaponInvTypes() const
{
    return AllowMixedWeaponInvTypes;
}
...
void Transmogrification::LoadConfig(bool reload)
{
    ...
    AllowNonusableItems = sConfigMgr->GetBoolDefault("Transmogrification.CanTransmogNonusableItems", true);
    ...
    AllowMixedWeaponInvTypes = sConfigMgr->GetBoolDefault("Transmogrification.AllowMixedWeaponInvTypes", true);
